...
Feature: DiWoo sitemap
  ...
  # ---------------------------------------------------------------------------
  # Gaps vs manual testscript 10 (matrix rows 209-226). Membership, metadata,
  # and change-propagation scenarios require a short sitemap cache on the
  # burgerportaal (`SITEMAP_CACHE_DURATION_HOURS=0` or ~1min) so seed→assert is
  # deterministic. Set `SITEMAP_CACHE_WAIT_MS` in .env for refetch steps.
  # Landelijke (non-zelf_toegevoegd) publisher seeds are used — ODBP skips
  # zelf_toegevoegd publishers. Before({tags:'@blocked'}) skips with reason.
  ...
  # Prerequisite: short sitemap cache override + SITEMAP_CACHE_WAIT_MS.
